...
[source,js]
----
const { IAmIdRouter } = require('@santander/iamid-provider') <1>
...
[source,js]
----
const { resolvers : { proxyClaimResolvers } } = require('@santander/iamid-provider')
...
Inside `utils` module we have a couple of utilities, `normalizePhoneNumber` and `Sensitive`. The idea is to add more utils in the future in this namespace.

.How to use normalizePhoneNumber?
[source,js]
----
const { utils: { normalizePhoneNumber } } = require('@santander/iamid-provider')
const phone = normalizePhoneNumber('636656767') <1>
console.log(phone) <2>
----
<1> This util just normalize a phone string to E164 format.
<2> The console will show `+44636656767`

.How to use Sensitive?
[source,js]
----
const { utils: { Sensitive } } = require('@santander/iamid-provider') <1>
const anObject = {
    user: 'hello',
    password: new Sensitive('my awesome password') <1>
  }
console.log(JSON.stringify(anObject)) <2>
----
<1> This utility a wrapper for sensitive information to avoid that accidentally can be printed in a log for exmaple.
<2> We just wrap the value in a `Sensitive` object.
<3> When try to show in the console we will find just `{"user":"hello"}`
