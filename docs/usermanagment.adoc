...
To work with `Users` we need an initialize instance of the `repositories` factory, and to create an instance just:
...
This `uid` passed as first parameter in the resolve method is the user unique identifier in the OP server, or what is the same, the returned uid by the `insertOrUpdate` method. iThe obtained object is an instance of `User` that is the root object that conatin all the `Connection` objects for a given end user in the OP server. To obtain the authoriztaion data just need to use the `getConnection(systemType)` method as:
