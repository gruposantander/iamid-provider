...
IAMID is based in https://github.com/panva/node-oidc-provider[oidc-provider] that implement lot of specification (can be consult in the product github) but the goal of this implementation is the IAMID profile so several specficiation/fetaure has been limited or deactivated and another ones added to the implementation, the supported specs/features are:
...
IAMID is provided as a library for implement an OP, so for example using `npm` you can install in your project using:

[source,bash]
----
npm install --save aimid
----
...
A good start point could be to use the Santander-uk project as base and modify the code there.
