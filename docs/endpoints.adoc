...
To execute the login logic a client or connector can be used to authenticate the user against an external API and store the authentication data in the Session Manager as described in <<connectors.adoc#,Implement the connectors>> documentation.

Also task related with validate user input should be implement in this endpoint.

After all the custom logic and the End User successful login into the system and authentication data is stored in Session Manager the endpoint should call to the service method `processLogin (userId, ctx, next)` to communicate `iamid-provider` that login has finished, this will make the authorization process progress to the next prompt. As you can see the koa Context `ctx` and `next` callback are passed to the method because from this point the `IAmIdRouter` will take the control so the call to the `processLogin` method should be the last statement of our login logic.
