...
----
const { resolvers: { Claim, ClaimResponse, Unresolved: { notFound } } } = require('iamid-provider') <1>
...
To allow connectors persist this session info `iamid-provider` provide a repository, see more in <<Sesion Management>>, but the exactly data that is store inside the `auth` object and how to refresh it's responsibility of the OP developer.
...
[source,js]
----
const { resolvers : { proxyClaimResolvers } } = require('iamid-provider') <1>
