...
The function accept two parameters, an `uid` that is the unique id identifier of the user in our OP server (see <<User session and credentials>> for more info) and a `requestedClaims` object that contain as keys the name of the claims that should be retrieved by the connector and as value a object with options that should be take into account while obtaining the claims.
...
[source,javascript]
----
async function exampleResolver (id, requestedClaims) { <1>
  // Code comes here
}
exampleResolver.claims = { claim1: { ial: 1 }, claim2: { ial: 2}, claim3: { ial: 2 } }  <2>
----
<1> Is an `async` function that accept `id` and `requestedClaims` params.
...
The connector function should return a `ClaimResponse` instance, this class contain for each claim that is requested using the `requestedClaims` parameter a `Claim` object. The way to create a `ClaimResponse` is passing a js Object with the claim name as key an a `Claim` instance as value (See examples below)

A `Claim` object contain an array with the resolved values and another array with the unresolved types. Resolved and unresolved values are arrays because one connector can return more than one value for each claim, for example in the case of the _"phone_number"_ an End-User can have more than one value registered, this array is returned and presented to the user in the consent screen so End-User can select the value that want to share with the RP.
...
To allow connectors persist this session info `iamid-provider` provide a repository, see more in <<usermanagment.adoc#,User authorization data management>>, but the exactly data that is store inside the `auth` object and how to refresh it's responsibility of the OP developer, when calling to the connector function the `uid` passed as input will be used to obtain the `auth` info from the user repository.

If you review the code in the resolver develop in the reference project https://uk-gitlab.almuk.santanderuk.corp/verified-id/op-server[Santander-uk project] you will notice that instead of a `uid` the resolvers has as input the `auth`, this is because in the resolver there are a refresh logic that already extract the info from the user repository and a function to wrap this behavior has been created before call the proxy, this avoid to call in each different resolver the same logic.
...
- Choose the correct connectors based on the requested claims (try to make avoid unended calls)
