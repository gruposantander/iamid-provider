...
function proxyResolvers (resolvers) {
  const resolverListMap = resolvers.reduce((map, resolver) => {
    resolver.claims.forEach(({ name, max_ial: max }) => {
  ...
  async function resolve (auth, requestedClaims) {
    ...
    const resolverSet = requestedClaims.reduce((set, { name, ials }) => {
    ...
    const promises = filteredResolvers.map(async (resolver) => {
      ...
      try {
        ...
        return new ClaimResponse(filteredRequestedClaims.map(({ name }) => [name, INTERNAL_ERROR_CLAIM]))
      }
    })
