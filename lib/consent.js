...
class Consent {
   ...
   * @returns {Consent}
   */
  static createConsent (clientId, requestUri, userId, claims, scopes) {
    return new Consent(nanoid(), requestUri, clientId, userId, claims, scopes)
  }

  static fromJSON (obj) {
    if (!obj) return null
    const { id, requestUri, clientId, userId, claims, scopes, accepted } = obj
    return new Consent(id, requestUri, clientId, userId, Claims.fromJSON(claims), scopes, accepted && Accepted.fromJSON(accepted))
  }

  constructor (id, requestUri, clientId, userId, claims, scopes, accepted = null) {
    this.id = id
  ...
  resolve (accepted) {
    ...
    return new Consent(this.id, this.requestUri, this.clientId, this.userId, claims, scopes, accepted)
  }
