...
class Consent {
  ...
  static createConsent (clientId, requestURI) {
    return new Consent({ id: nanoid(), requestURI, clientId })
  }

  static fromJSON (obj) {
    if (!obj) return null
    const { claims, accepted } = obj
    return new Consent(obj, { claims: Claims.fromJSON(claims), accepted: Accepted.fromJSON(accepted) })
  }

  constructor (old, props) {
    const { id, requestURI, clientId, claims, scopes, interactionId, userId, resolved, accepted } = { ...old, ...props }
    this.id = id
  ...
  setInteractionId (interactionId) {
    return new Consent(this, { interactionId })
  }

  resolveClaims (userId, claims, scopes) {
    return new Consent(this, { claims, scopes, userId, resolved: true })
  }
