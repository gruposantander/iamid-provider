...
const { assert, isObject, isStringArray, ValidationError } = require('./validation')
...
class AcceptedGroup {
  ...
  static fromJSON (name, group, validate = false) {
    ...
    if (validate) {
      assert(isStringArray(approvedClaims), 'approved_claims must be an array of strings')
      assert(isStringArray(approvedAssertions), 'approved_assertions must be an array of strings')
...
class Accepted {
  ...
  static fromJSON (obj, validate = false) {
    ...
    if (validate) {
      assert(isObject(idToken), 'id_token must be an object')
      assert(isObject(userInfo), 'userinfo must be an object')
      assert(isStringArray(scopes), 'approved_scopes must be an array of strings')
    }
    return new Accepted(
...
function validateAccepted (group, groups, approved) {
  ...
  Object.entries(claims).forEach(([name, index]) => {
    ...
    if (!resolved[name]) {
      throw new ValidationError(`${name} could not be accepted as it has not been requested at ${group}`)
    }
    if (index >= claim.result.length) {
      throw new ValidationError(`index ${index} is not valid for ${name} claim at ${group}`)
    }
  })
  Object.entries(assertions).forEach(([name, index]) => {
    if (!assertionClaims[name]) {
      throw new ValidationError(`${name} could not be accepted as it has not been requested at ${group} assertions`)
    }
  })
}
...
class AssertionClaim {
  ...
  static fromJSON (obj, validate = false) {
    if (validate) {
      assert(isObject(obj), 'assertion claim must by an object')
      const { purpose, assertion } = obj
      assert(purpose === undefined || typeof purpose === 'string', 'purpose must by a string')
      assert(isObject(assertion), 'assertion should be an object')
    }
...
class AssertionGroupClaim {
  ...
  static fromJSON (obj = {}, validate = false) {
    if (validate) {
      assert(isObject(obj), 'group should be an object')
    }
...
class Consent {
  ...
  claimsFor (section) {
    ...
    const result = Object.entries(claims).reduce((obj, [name, index]) => {
      if (index !== -1) {
        obj[name] = group[name].raw_result.value[index]
      }
      return obj
    }, { sub: this.userId })
...
module.exports = {
  Consent, Accepted, AcceptedGroup, ValidationError, Claims, ClaimGroup
}
