...
// TODO should we do a from JSON of the base claim or how to do here the validations?
class BaseClaim {
  constructor (options) {
    const { purpose, ial = DEFAULT_IAL, result, raw_result, unresolved, essential } = options
  ...
  static validate (obj) {
    const { purpose, essential, ial } = obj
...
class AssertionClaim extends BaseClaim {
  constructor (options) {
    super(options)
...
class Consent {
   ...
   * @returns {Consent}
   */
  static createConsent (clientId, requestUri) {
    return new Consent(nanoid(), requestUri, clientId)
  }

  static fromJSON (obj) {
    if (!obj) return null
    const { id, requestUri, clientId, userId, claims, scopes, interactionId, accepted, resolved } = obj
    return new Consent(id, requestUri, clientId, Claims.fromJSON(claims), scopes, interactionId, userId, resolved, accepted && Accepted.fromJSON(accepted))
  }

  constructor (id, requestUri, clientId, claims = null, scopes = null, interactionId = null, userId = null, resolved = false, accepted = null) {
    this.id = id
  ...
  setInteractionId (interactionId) {
    return new Consent(this.id, this.requestUri, this.clientId, this.claims, this.scopes, interactionId)
  }

  resolveClaims (userId, claims, scopes) {
    return new Consent(this.id, this.requestUri, this.clientId, claims, scopes, this.interactionId, userId, true)
  }
   ...
   */
  accept (accepted) {
    const { id, requestUri, clientId, claims, scopes, interactionId, userId, resolved } = this
    validateAccepted('id_token', claims, accepted)
    validateAccepted('userinfo', claims, accepted)
    return new Consent(id, requestUri, clientId, claims, scopes, interactionId, userId, resolved, accepted)
  }
