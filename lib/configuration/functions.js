'use strict'
const { interactionPolicy: { base, Check } } = require('@santander/oidc-provider')
...
module.exports = function (configuration) {
  ...
  function findAccount (ctx, id) {
    return {
      accountId: id,
      async claims (use) {
        const consents = await ctx.app.repositories.getRepository('Consents')
        // TODO if consent is missing
        const consent = await consents.findOne({ clientId: ctx.oidc.client.clientId, userId: id }, Consent.fromJSON)
