...
const { SessionNotFound } = require('@gruposantander/oidc-provider').errors
const { wrap } = require('./sensitive')
const { UnknownOperatorException, compile, types } = require('@gruposantander/json-el')
...
types.set('phone_number', (value) => normalize(value))
...
class IAmIdRouter extends Router {
  constructor (configuration, repositories, resolver, router) {
    ...
    function executeAssertions ({ assertion_claims: obj = {} }, response) {
      Object.keys(obj).forEach((key) => {
        ...
        try {
          ...
          if (err instanceof UnknownOperatorException) {
        ...
        value.match = !!value.result.length
        obj[key] = value
      })
    }
    ...
    async function interaction (ctx) {
      ...
      if (prompt.name === 'login' || !accountId) {
        ...
        ctx.body = {
          interaction: 'login',
          interaction_id: uid,
          interaction_path: `${INTERACTION_PATH}/${uid}/login`,
        ...
        const consent = await consents.findOne({ interactionId: uid }, Consent.fromJSON)
        const updated = consent.resolveClaims(accountId, claims, scopes)
        ...
        const body = {
          interaction: 'consent',
          interaction_id: uid,
          interaction_path: `${INTERACTION_PATH}/${uid}/consent`,
          redirect_uri: params.redirect_uri,
          state,
    ...
    async function initiateConsent (ctx, next) {
      await next()
      if (ctx.status === 201) {
        const {
          body: { request_uri: requestURI },
          oidc: { params: { claims: claimsStr, client_id: clientId } }
        } = ctx
        const consent = Consent.createConsent(clientId, requestURI)
        const consents = await repositories.getRepository(CONSENTS)
        await consents.save(consent)
        // TODO we need to pass claims already parsed to the Consent constructor?
    ...
    async function updateConsent (ctx, next) {
      await next()
      if (ctx.status === 302 && !ctx.response.headers.location.includes('error')) {
        const requestURI = ctx.URL.searchParams.get('request_uri')
        const interactionId = ctx.response.headers.location.slice(INTERACTION_PATH.length + 1)
        const consents = await repositories.getRepository(CONSENTS)
        const consent = await consents.findOne({ requestURI }, Consent.fromJSON)
        const updated = consent.setInteractionId(interactionId)
        // TODO when "auto consent" is implemented here we need to check the accountId.
        await consents.save(updated)
      }
    }

    this
      .post(configuration.routes.pushed_authorization_request, initiateConsent)
      .get(AUTH_PATH, updateConsent, forceConsent, adaptCookies)
  ...
  async processConsent (consentData, ctx, next) {
    ...
    const consent = await consents.findOne({ interactionId: interaction.uid }, Consent.fromJSON)
    ...
    const updated = consent.accept(accepted)
    await consents.save(updated)
