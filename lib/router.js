  ...
  constructor (configuration, repositories, resolver, router) {
    ...
    async function interaction (ctx) {
      ...
      if (prompt.name === 'login' || !accountId) {
        ...
        const consent = Consent.createConsent(params.client_id, undefined, accountId, claims, scopes)
    ...
    async function initiateConsent (ctx, next) {
      ...
      if (ctx.status === 201) {
        const {
          ...
          oidc: { params: { claims: claimsStr, scopes, clientId } }
        } = ctx
    ...
    this
      .get(AUTH_PATH, foo, forceConsent, adaptCookies)
