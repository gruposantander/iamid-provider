...
const { UnknownOperatorError, filterValues, compile } = require('./assertion-lang')
const masked = require('./mask')
const { Consent, Accepted, ValidationError } = require('./consent')
...
class IAmIdRouter extends Router {
  constructor ({ configuration, provider, repositories }, connector) {
    super()
    this.repositories = repositories
    const AUTH_PATH = configuration.routes.authorization
    const assertionClaimsSupported = new Map(Object.entries(provider
      .configuration('discovery.claims_in_assertion_claims_supported')))
    const claimsSupported = provider.configuration('claimsSupported')
    ...
    const { interaction: INTERACTION_COOKIE_NAME } = provider.configuration('cookies.names')
    const COOKIE_NAMES = [INTERACTION_COOKIE_NAME]
    ...
    function applyResponse (obj, response) {
      Object.keys(obj).filter((name) => name !== 'assertion_claims').forEach((key) => {
        const claim = response.get(key)
        // TODO report an error
        const { resolved, unresolved } = claim
        const value = obj[key] || {}
        value.raw_result = wrap(resolved)
        value.result = resolved.map((item) => applyMask(key, item))
    ...
    function executeAssertions ({ assertion_claims: obj = {} }, response) {
      Object.keys(obj).forEach((key) => {
        ...
        try {
          value.raw_result = wrap(filterValues(resolved, obj[key].assertion, assertionClaimsSupported.get(key)))
        } catch (err) {
        ...
        value.result = value.raw_result.value.map((item) => applyMask(key, item))
        obj[key] = value
      })
    }

    async function resolveClaims (claimRequest, uid) {
      const claims = (claimRequest && JSON.parse(claimRequest)) || {}
      ...
      const set = new Set([
        ...
        ...Object.keys(idToken.assertion_claims || {}),
        ...Object.keys(userInfo.assertion_claims || {})
      ])
      set.delete('assertion_claims')
      const claimsNames = Array.from(set)
      const response = await connector.resolve(uid, claimsNames)
    ...
    this
      ...
      .use(errorHandler)
      .post('/logout', logout)
      .use(INTERACTION_PATH + '/:interactionId', preInteractionChecks, bodyParser(), adaptCookies)
